...
## read the variable names from features.txt file into R
...
## assemble train dataset, labeling the dataset with descriptive variable names
...
subjecttraindata <- read.table("./train/subject_train.txt", col.names = c("SubjectID"))
  ## combine the components of the train dataset together
traindata <- cbind(subjecttraindata, ytraindata, xtraindata)

## assemble test dataset, labeling the dataset with description variable names
...
subjecttestdata <- read.table("./test/subject_test.txt", col.names = c("SubjectID"))
  ## combine the components of the test dataset together
...
## extract the mean and standard deviation for each measurement (and the Activity and SubjectID columns)
sdata <- data[colIndex]
